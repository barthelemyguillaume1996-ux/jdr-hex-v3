...
export function applyCastMessage(msg) {
    ...
    switch (type) {
        ...
        case "REQUEST_SNAPSHOT": {
            // ✅ SERVEURLESS: Le GM doit savoir qu'on lui demande l'état
            if (typeof window !== "undefined") {
                try { window.dispatchEvent(new CustomEvent("cast:request_snapshot")); } catch { }
            }
            noteRx();
            return;
        }
...
function send(msg) {
    ...
    // ✅ PRIORITÉ LOCALE: On envoie toujours via BroadcastChannel (latence zéro)
    postLocal(msg);

    // Et au serveur si connecté (pour backup ou remote)
...
if (typeof window !== "undefined") {
    ...
    // Auto-connect si URL mémorisée / globale / ou defaut local
    try {
        let url =
