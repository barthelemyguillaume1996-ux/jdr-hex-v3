...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case "SET_VIEWPORT": {
            ...
            if (!w || !h) return state;
            // Optim: si inchangé, on ne touche à rien
            if (state.viewport && state.viewport.w === w && state.viewport.h === h) return state;
            return { ...state, viewport: { w, h } };
        }
...
export function StateProvider({ children }) {
    ...
    const dispatchCast = React.useCallback((action) => {
        dispatch(action);
    }, []);
